...
def save_S_vs_t(theta,L_list,BC,evolution_type,scrambling_type,root_direc,A=0,depth=4,scr_depth=1):
    ...
    pl.legend()
    save_file = get_filename(t_scr,evolution_type,scrambling_type,root_direc) + '/figures/S_vs_t_A='+str(A)+'_BC='+str(BC)+'/'
...
def half_system_S_vs_t(theta,L_list,BC,evolution_type,scrambling_type,root_direc,depth=4,scr_depth=1):
    ...
    for L in L_list:
        ...
        zz,p = get_slope(xdata[2*L:3*L],np.log(ydata[2*L:3*L]))
        slope.append(zz[0])
        avg_ent.append(np.average(ydata[-2*L:]))
        # print(entropy[-1])
        pl.errorbar(xdata[:]/L,ydata[:],yerr=err[:],ls='-',marker='o',label='L={}'.format(L))
        pl.plot(xdata[:]/L,np.exp(p(np.array(xdata))),'k',ls=':')
    ...
    pl.title(r'$p={}$'.format(theta),fontsize=16)
    pl.legend()
    save_file = get_filename(t_scr,evolution_type,scrambling_type,root_direc) + '/figures/half_system_BC='+str(BC)+'/'
    if not os.path.isdir(save_file):
        os.makedirs(save_file)
    pl.savefig(save_file+'S_vs_t_theta={}.pdf'.format(theta))
    pl.close(1)

    pl.figure(2)
    pl.plot(L_list[:],[i for i in avg_ent],'-o')
    pl.ylabel(r'$S_A$',fontsize=16)
    pl.xlabel(r'$L$',fontsize=16)
    # pl.yscale('log')
    pl.savefig(save_file+'S_vs_L_theta={}.pdf'.format(theta))
    pl.close(2)