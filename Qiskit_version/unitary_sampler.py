...
def matchgate_sampler(rng: np.random.default_rng):
    ...
    phase2 = 5
    U[0, 0] = 1 
    U[3, 3] = (2*rng.integers(0,2)-1) 
    U_10 = haar_qr(2,rng)
    U_10 = (2*rng.integers(0,2)-1)*U_10/np.sqrt(np.linalg.det(U_10))
    U[np.array([1,2]).reshape((2,-1)),np.array([1,2])] = U_10
    U = U*global_phase
    return U
...
def get_U_1_unitary_circuit(seed_unitary, number_of_gates,which_U='haar'):
    ...
    filename = 'unitary_gates_data/U(1)'+U_label+'/N=' + str(4) + '_seed=' + str(seed_unitary)
    if os.path.isfile(filename):
        with open(filename, 'rb') as f:
            unitary_gates, unitary_rng = pickle.load(f)
